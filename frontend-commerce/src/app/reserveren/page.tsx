  ...
  const fetchAvailableDates = useCallback(async (year: number, month: number, party: number) => {
    ...
    try {
      const res = await fetch(
        `${API}/reservations/available-dates?year=${year}&month=${month + 1}&partySize=${party}`,
      );
          ...
          {step === 'done' && (
            <div className="reserv-card reserv-card--success">
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2rem', flexWrap: 'wrap' }}>
                ...
                <a className="btn btn--ghost" href="/">Terug naar home</a>
              </div>
            </div>
          )}

...
export function TableDialog({ open, table, defaultRoomId, rooms, onClose, onSaved }: Props) {
    ...
    if (open) {
      setForm(
        table
          ...
          : { name: '', capacity: 1, roomId: defaultRoomId ?? rooms[0]?.id ?? '' },
      );
      setError(null);
    }
  }, [open, table, defaultRoomId, rooms]);
